...
exports.HttpStatus = {};
const map = {};
for (let i = 0, len = exports.httpStatuses.length; i < len; i += 1) {
    const status = exports.httpStatuses[i];
    exports.HttpStatus[status.code] = status;
    exports.HttpStatus[status.name] = status;
    map[status.code] = status;
    map[status.name] = status;
    map[status.name.replace(/_/g, '')] = status;
}
const nonAlpha = /[^A-Za-z]+/g;
exports.getHttpStatus = (codeOrName) => {
